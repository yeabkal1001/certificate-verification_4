...
import { bulkUploadSchema, type BulkUploadFormData } from "@/lib/validation/schemas"
...
export function BulkUploadForm({ onClose, onCertificatesAdded }: BulkUploadFormProps) {
	...
	// Add react-hook-form for file and template selection
	const {
		register,
		handleSubmit,
		setValue,
		...
		defaultValues: { file: undefined, templateId: "" },
	})
	...
	const uploadedFile = watch("file") as File | undefined
	...
	const onSubmit = async (formData: BulkUploadFormData) => {
		...
		try {
			...
			toast({
				...
				description: `${newCertificates.length} certificates have been processed using ${mockTemplates.find((t) => t.id === formData.templateId)?.name} template`,
			})
		...
		<div className="space-y-6">
			<Tabs defaultValue="csv" className="w-full">
				<TabsList className="grid w-full grid-cols-2">
					<TabsTrigger value="csv" data-testid="csv-upload-tab">
						CSV Upload
					</TabsTrigger>
				...
				<TabsContent value="csv" className="space-y-6">
					<form onSubmit={handleSubmit(onSubmit)} data-testid="bulk-upload-form">
						<Card>
							<CardHeader>
								<CardTitle>CSV File Upload</CardTitle>
								<CardDescription>
									Upload a CSV file containing multiple certificate records
								</CardDescription>
							</CardHeader>
								...
								<div className="space-y-4">
									<div>
										...
										<p className="text-sm text-muted-foreground mb-2">
											Choose the template that will be used for all certificates in
											this batch
										</p>
										<select
											id="templateId"
										...
										{errors.templateId && (
											<span className="text-red-500 text-xs" data-testid="template-error">
									...
									<div>
										...
										<div className="mt-2">
											<label htmlFor="csv-upload" className="cursor-pointer">
												<div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center hover:border-muted-foreground/50 transition-colors" data-testid="csv-drop-area">
													{uploadedFile ? (
											...
											{errors.file && (
												<span className="text-red-500 text-xs" data-testid="file-error">
													{errors.file.message}
												</span>
											)}
										</div>
													...
													<tbody>
														{csvData.slice(0, 5).map((row, index) => (
															<tr key={index} className="border-b">
																<td className="p-1">
																	{row.certificateId || "Auto-generated"}
																</td>
																<td className="p-1">
																	{row.recipientName ||
																		row["Recipient Name"] ||
																...
																<td className="p-1">
																	{row.courseName || row["Course Name"] || "N/A"}
																</td>
																<td className="p-1">
																	{row.issueDate || row["Issue Date"] || "N/A"}
																</td>
																<td className="p-1">
									...
									<div className="bg-muted/50 p-4 rounded-lg">
										...
										<ul className="text-sm text-muted-foreground space-y-1">
											<li>
												• certificateId (optional - will be auto-generated in format
												CERT-YYYY-NNNN)
											</li>
						...
						<div className="flex justify-end gap-4 mt-4">
							...
							<Button
								...
								disabled={
									isLoading || !uploadedFile || !selectedTemplate || csvData.length === 0
								}
				...
				<TabsContent value="manual" className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Enter multiple certificates manually using the form interface
							</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="text-center py-8">
								<p className="text-muted-foreground mb-4">
									Manual bulk entry form will be available in a future update. For
									now, please use the CSV upload option or create certificates
									individually.
								</p>
