...
import type { DraggableEventHandler, DraggableData } from "react-draggable"
...
// Add signatureBase64 to CertificateTemplate type
type CertificateTemplateWithSignature = CertificateTemplate & { signatureBase64?: string }
...
export function TemplateManagement() {
  ...
  // Handle drag position for signature and QR code
  const handleDragPosition = (key: "signature" | "qr", x: number, y: number) => {
    if (selectedTemplate) {
      const updatedPosition = { x: Math.round(x), y: Math.round(y) }
      setSelectedTemplate({
        ...selectedTemplate,
        textPositions: {
          ...selectedTemplate.textPositions,
          [key]: updatedPosition,
        },
      })
    }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  {/* Draggable Signature */}
                  <Draggable
                    position={{ x: selectedTemplate.textPositions.signature.x, y: selectedTemplate.textPositions.signature.y }}
                    bounds="parent"
                    onStop={(_e: any, d: any) => handleDragPosition("signature", d.x, d.y)}
                  >
                    <div data-testid="signature-draggable" className="absolute cursor-move">
                      <img src={(selectedTemplate as any).signatureBase64 || "/placeholder-user.jpg"} alt="Signature" className="h-12 w-auto" />
                      <input
                        type="file"
                        accept="image/*"
                        style={{ display: "none" }}
                          ...
                          if (file) {
                            ...
                            reader.onload = (event) => {
                              setSelectedTemplate({
                                ...
                                signatureBase64: event.target?.result as string,
                              })
                            }
                            reader.readAsDataURL(file)
                          }
                      ...
                      />
                      <label htmlFor="signature-upload" className="block text-xs text-blue-600 cursor-pointer mt-1" data-testid="signature-upload-label">Upload Signature</label>
                    </div>
                  </Draggable>

                  {/* Draggable QR Code */}
                  <Draggable
                    position={{ x: selectedTemplate.textPositions.qr.x, y: selectedTemplate.textPositions.qr.y }}
                    bounds="parent"
                    onStop={(_e: any, d: any) => handleDragPosition("qr", d.x, d.y)}
                  >
                    <div data-testid="qr-draggable" className="absolute cursor-move">
                      <QRCodeCanvas value={`https://yourdomain.com/verify?code=DEMO-QR`} size={64} />
                    </div>
                  </Draggable>
                </div>
              </div>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <h4 className="font-medium">Live Preview</h4>
              <div
                className="aspect-[3/2] border rounded-lg p-4 flex flex-col justify-center items-center text-center relative"
                style={{
              ...
              >
                ...
                {/* Draggable Signature */}
                <Draggable
                  position={{ x: newTemplate.textPositions.signature.x, y: newTemplate.textPositions.signature.y }}
                  bounds="parent"
                  onStop={(_e, d) => handleDragPosition("signature", d.x, d.y)}
                >
                  <div data-testid="signature-draggable" className="absolute cursor-move">
                    <img src={newTemplate.signatureBase64 || "/placeholder-user.jpg"} alt="Signature" className="h-12 w-auto" />
                    <input
                      type="file"
                      accept="image/*"
                      style={{ display: "none" }}
                      id="signature-upload-create"
                      data-testid="signature-upload-input-create"
                      onChange={e => {
                        const file = e.target.files?.[0]
                        if (file) {
                          const reader = new FileReader()
                          reader.onload = (event) => {
                            setNewTemplate({ ...newTemplate, signatureBase64: event.target?.result as string })
                          }
                          reader.readAsDataURL(file)
                        }
                      }}
                    />
                    <label htmlFor="signature-upload-create" className="block text-xs text-blue-600 cursor-pointer mt-1" data-testid="signature-upload-label-create">Upload Signature</label>
                  </div>
                </Draggable>

                {/* Draggable QR Code */}
                <Draggable
                  position={{ x: newTemplate.textPositions.qr.x, y: newTemplate.textPositions.qr.y }}
                  bounds="parent"
                  onStop={(_e, d) => handleDragPosition("qr", d.x, d.y)}
                >
                  <div data-testid="qr-draggable" className="absolute cursor-move">
                    <QRCodeCanvas value={`https://yourdomain.com/verify?code=${uuidv4()}`} size={64} />
                  </div>
                </Draggable>
              </div>
