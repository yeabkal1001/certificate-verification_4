...
export default function CertificateDetailsPage() {
  ...
  const handleDownload = async (certificateId: string, format: "pdf" | "png") => {
    try {
      if (format === "pdf") {
        const blob = await certificatesApi.download(certificateId, "pdf")
        ...
        // For PNG, generate it client-side
      ...
      toast({
        ...
        description: `Certificate download for ${certificateId} started`,
      })
    } catch (error) {
      toast({
        title: "Download Failed",
        description: `Could not download certificate: ${error}`,
        variant: "destructive",
      })
    }
  }
              ...
              <CardContent className="space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-4">
                    <Button onClick={() => handleDownload(certificate.id, "pdf")}>
                      <Download className="h-4 w-4 mr-2" />
                      Download PDF
                    </Button>

                    <Button variant="outline" onClick={() => handleDownload(certificate.id, "png")}>
                      <Download className="h-4 w-4 mr-2" />
                      Download PNG
                    </Button>
